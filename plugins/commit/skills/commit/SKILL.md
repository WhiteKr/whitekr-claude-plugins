...
2. diff hunk 들을 의도 기준으로 그룹핑. 그룹 요약을 사용자에게 보여주고 어떤 그룹을 이번 commit 에 포함할지 확인.
...
6. Commit 후 `git status` 로 잔여 변경 확인. 남았다면 추가 commit 필요한지 사용자에게 안내.

## 메시지 스타일

**언어**: `git log --oneline -20` 의 기존 commit 다수가 쓰는 언어로 작성한다 (한국어 레포면 한국어, 영어 레포면 영어). 혼재 시 더 최근 다수를 따르고, 판단이 안 서면 사용자에게 확인. 톤도 참고하되 아래 가드가 우선한다 — 최근 commit이 모두 AI-스러운 톤이면 모방 루프가 만들어지므로 가드로 끊는다.
