...
현재 디렉토리(`$CWD`) 하위의 모든 git 레포와 Claude Code settings에 등록된 외부 워크스페이스를 찾아, outdated된 레포만 선택적으로 `git pull`을 실행하세요.
...
> **출력 규칙**: 절차 진행 중 사용자에게 텍스트를 출력하지 않는다. "N개의 레포를 발견했습니다", "이 레포는 detached HEAD 상태입니다", "CLI가 설치되어 있습니다" 같은 중간 상태 보고를 하지 말 것. 도구 호출만 연속 실행하고, 최종 결과 보고 테이블만 출력한다. 단, 사용자 확인이 필요한 에러나 판단이 필요한 상황은 예외.
...
```
Glob(pattern: "**/.git/HEAD", path: "$CWD")
```
...
2. `$CWD/.claude/settings.json` → `permissions.additionalDirectories`
3. `$CWD/.claude/settings.local.json` → `permissions.additionalDirectories`

모든 소스에서 발견된 경로를 합친다 (중복 제거). 각 외부 디렉토리에 대해:
...
**인자 필터링**: 스킬에 인자가 전달된 경우 (예: `/pull BE.Main`), 해당 문자열을 포함하는 레포 경로만 남긴다. 인자가 없으면 전체 목록을 사용한다.
...
**Submodule 중복 제거**: 결과에서 한 레포 경로가 다른 레포 경로의 하위인 경우, 하위 경로가 submodule일 가능성이 높다. 이런 경우는 제거하지 않되, triage에서 자연스럽게 처리된다 (submodule은 보통 upstream이 설정되어 있지 않아 NO_UPSTREAM으로 분류됨).

### 2. Triage + Pull (외부 스크립트 — 자동 승인)

`triage.sh`에 레포 경로들을 인자로 전달한다. 이 스크립트는 모든 레포를 병렬로 fetch, 상태 확인, 그리고 OUTDATED 레포는 즉시 pull까지 수행한다.
...
로컬 변경이나 에러가 있는 레포, 또는 📥 Updated 레포는 테이블 아래에 상세 내역을 별도 섹션으로 출력:

### (레포명) 상세
- **pulled**: diff 내용(코드 변경)과 커밋 메시지(의도)를 종합 분석하여, 실제로 무엇이 어떻게 변경되었는지를 한국어로 서술한 요약을 작성한다. 커밋 해시나 메시지를 그대로 나열하지 말 것. 각 변경 항목에 관련 파일명을 괄호로 표시한다. triage.sh 출력의 NEW_COMMITS 블록에 `(작성자명)` 이 포함되므로, 변경 요약 끝에 `— by 작성자명` 형태로 누가 변경했는지 표시한다. 여러 작성자가 있으면 각각 표시한다.
  - 좋은 예: `Electron webview에서 ziowww 페이지 로드 시 overflow 스타일이 hidden으로 고정되어 스크롤이 불가능하던 문제를 auto로 변경하여 수정 (WindowManager.tsx) — by 홍길동`
  - 나쁜 예: `` `d0c7d87` fix: Electron에서 ziowww 페이지 스크롤 불가 문제 해결 ``
- **uncommitted**: 파일 목록 (status --porcelain 출력)
- **unpushed**: 커밋 목록 (log --oneline 출력)
- **error**: 에러 내용
- **timeout**: fetch 타임아웃 발생

### Submodule 업데이트 가능

submodule에 원격 업데이트가 있는 레포가 있으면 별도 섹션으로 보고:

| 레포 | Submodule | 새 커밋 수 |
|------|-----------|-----------|
| (레포 경로) | (submodule 경로) | N commits |

각 submodule의 새 커밋 목록 (log --oneline)도 함께 표시.
